...
# 含义：只处理Y坐标小于500的支付金额文本（即页面上方的支付字段）
PAYMENT_AMOUNT_Y_THRESHOLD = 500
...
def _find_payment_amount_block(
    ...
    参数说明：
        text_infos (List[TextInfo]): 所有文本块的信息列表
                                     会在这个列表中搜索金额块
        payment_block (TextInfo): 包含"向您支付的金额"的文本块
                                 用于计算目标金额块的搜索范围
...
def merge_text_blocks(
    ...
    try:
        ...
        for text_info in text_infos:
            if "向您支付的金额" in text_info.text and text_info.center_y < 1000:
        ...
        if payment_text_info:
            ...
            target_y = payment_text_info.center_y + 500
            target_x_range = (target_x - PAYMENT_OFFSET_X_TOLERANCE, target_x + PAYMENT_OFFSET_X_TOLERANCE)
            ...
            for i, amount_info in enumerate(amount_infos):
                ...
                y_match = target_y_range[0] <= amount_info.center_y <= target_y_range[1]
                ...
                if x_match and y_match:
                    ...
                    logger.debug(f"找到'向您支付的金额'对应的金额: {amount_info.text}")
                    break

        # ===== 步骤5：按Y坐标从小到大排序text_infos（自然顺序） =====
        text_infos_sorted = sorted(text_infos, key=lambda x: x.center_y)
        ...
        for text_info in text_infos_sorted:
            ...
            for i, amount_info in enumerate(amount_infos):
                if i not in processed_amounts:
                    # 检查Y坐标是否在容差范围内
                    if abs(text_info.center_y - amount_info.center_y) <= y_tolerance:
                        matched_amounts.append((i, amount_info))

            # 如果有多个匹配，按Y中心坐标最接近排序
            if matched_amounts:
                matched_amounts.sort(
                    key=lambda x: abs(text_info.center_y - x[1].center_y)
                )
                # 标记这些金额为已处理
                for i, _ in matched_amounts:
                    processed_amounts.add(i)

            # 创建行数据
            line_data = [text_info]
            # 添加匹配的金额块
            for _, amount_info in matched_amounts:
                line_data.append(amount_info)

            # 特殊处理：如果这是"向您支付的金额"文本且找到了对应的金额，添加它
            if text_info == payment_text_info and payment_amount_info:
                line_data.append(payment_amount_info)

            # 按X坐标从左到右排序
            line_data.sort(key=lambda x: x.center_x)
        ...
        for i, amount_info in enumerate(amount_infos):
            if i not in processed_amounts:
                merged_lines.append([amount_info])
        ...
        # ===== 生成最终结果 =====
        result_text = '\n'.join(output_lines)
        logger.debug(f"文本块合并处理完成，共 {len(output_lines)} 行")
